...
메인 프로세스의 백그라운드 리프레셔가 이름 그룹별 현황을 미리 계산해
...
def group_fingerprint(group: list[Product]) -> str:
    """이름 그룹의 (id, updated_at) 직렬화 — 프로덕트 등록/수정으로 스냅샷이
    오래됐는지 판별하는 지문이다."""
    return "|".join(f"{p.id}:{p.updated_at}" for p in group)


def build_summary(name: str, group: list[Product], merged: list[dict]) -> dict:
...
def refresh_snapshots(session: Session) -> int:
    ...
    results: dict[str, tuple[str, dict, list[dict]]] = {}
    for name, group in groups.items():
        fingerprint = group_fingerprint(group)
        merged = test_status_service.merge_status(test_status_service.scan_group(group))
        results[name] = (fingerprint, build_summary(name, group, merged), merged)

    now = _utcnow()
    for name, (fingerprint, summary, detail) in results.items():
        row = session.get(TestStatusSnapshot, name)
        if row is None:
            session.add(
                TestStatusSnapshot(
                    name=name,
    ...
    if results:
        session.execute(
            delete(TestStatusSnapshot).where(
                TestStatusSnapshot.name.not_in(list(results))
            )
        )
    else:
        session.execute(delete(TestStatusSnapshot))
    session.commit()
    return len(results)


def load_summaries(session: Session) -> dict[str, tuple[dict, datetime]]:
    """이름 → (summary dict, generated_at) 맵. detail 컬럼은 로드하지 않는다(요약 폴링 경량화)."""
...
def load_detail(session: Session, name: str) -> list[dict] | None:
    """이름의 detail(TargetFileStatus dict 리스트). 스냅샷이 없으면 None."""
    return session.execute(
        select(TestStatusSnapshot.detail).where(TestStatusSnapshot.name == name)
    ).scalar_one_or_none()
