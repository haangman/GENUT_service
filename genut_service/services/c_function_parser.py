...
- C++: `namespace X {`/`extern "C" {` 블록 내부(투명 처리), `T Class::method(...)`
  ...
  클래스/구조체 본문 안의 인라인 멤버 함수(타입 정의 본문도 계속 스캔한다 —
  `= { ... }` 초기화 리스트는 여전히 비매치).
...
_TYPE_KEYWORD_RE = re.compile(r"\b(?:class|struct|union|enum)\b")


def _is_type_body(segment: str) -> bool:
    """`{`가 타입 정의 본문(class/struct/union/enum)인지 판정한다.

    타입 정의 본문은 건너뛰지 않고 계속 스캔해 **클래스/구조체 안의 인라인 멤버
    함수**를 찾는다. `=`가 있으면 초기화 리스트(`struct fops x = { ... }`,
    커널 ops 테이블 등)이므로 제외한다.
    """
    return "=" not in segment and _TYPE_KEYWORD_RE.search(segment) is not None
...
def extract_functions(text: str) -> list[FunctionSpan]:
    ...
    while i < n:
        ...
        if ch == "{":
            ...
            if _TRANSPARENT_RE.match(segment) or _is_type_body(segment):
                # namespace/extern "C" 블록과 타입 정의 본문(class/struct/union/enum):
                # 내부도 계속 스캔한다 — C++ 인라인 멤버 함수를 찾기 위함.
                # (닫는 `}`는 위의 ';}' 분기에서 세그먼트만 리셋하고 지나간다)
                seg_start = i + 1
                i += 1
                continue
            # 그 외(초기화 리스트, 분류 불가 블록 등)는 통째로 건너뛴다
