...
| 페이지 | 설명 |
|--------|------|
| 프로덕트 등록 | 테스트 생성 대상 repo 등록/수정. **프로젝트**(Ulysses/Thetis, 기본 Ulysses) 선택, 코드 저장 경로(**절대 경로 필수**) **다운로드 버튼**(git clone/pull), CMAKE_CONFIGURE/BUILD_CMD **실행 버튼**(체크아웃에서 시험 실행) — 다운로드·실행 로그는 폼 하단 **공용 로그창**에 누적. **자동 실행 모드**(주기 실행) 지원 — 대상 파일 미리보기·제외 패턴·CMakeLists 양식·패치 관리 |
...
| 터미널 | 서비스 실행 환경의 **인터랙티브 셸**(xterm.js + WebSocket + PTY) — 명령 실행·디버깅·파일 정리. 여러 탭으로 열고 닫기. 서비스가 Docker 컨테이너 안이면 그 컨테이너 셸. **Linux/WSL·Docker 전용**(Windows는 미지원 안내) |
...
| `FORM_CMD_TIMEOUT` | `600` | 프로덕트 폼 명령 실행 버튼 타임아웃(초) |
| `TERMINAL_ENABLED` | `true` | 웹 터미널 페이지 활성화(웹으로 셸 노출 — 신뢰 네트워크에서만) |
| `TERMINAL_SHELL` | `` | 터미널 셸(빈 값=`$SHELL`→`/bin/bash`→`/bin/sh` 자동) |
